...
export function ClientDetail() {
  ...
  const stats = useMemo(() => ({
    ...
    totalValue: activeContracts.reduce((sum, c) => sum + (c.qty_remaining_kg * c.price_sell), 0)
  }), [activeContracts]);
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main className="max-w-4xl mx-auto px-6 py-16 text-center">
          ...
          <p className="text-muted">Aucun client avec le code "{code}"</p>
        </main>
      </div>
    );
  }
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-primary">{clientName}</h1>
            ...
            <div className="space-y-3">
              {completedContracts.slice(0, 5).map((contract, idx) => (
                ...
                >
                  ...
                  <p className="text-sm text-muted mt-1">
                    {formatDate(contract.date_start)} - {formatDate(contract.date_end)} | {formatWeight(contract.qty_contracted_kg)} livrees
                  </p>
                </div>
              ))}
            </div>
